...
from src.dbt.generate import generate_manifest
...
def run(config: Config) -> int:
    ...
    # 1. Get manifest (auto-generate or use provided path)
    if config.auto_generate_manifest:
        print(f"Generating manifest from dbt project: {config.project_dir}")
        manifest_path = generate_manifest(config.project_dir, config.dbt_version)
    else:
